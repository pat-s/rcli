...
if [[ $(diff tests/centos7/test-out.txt /tmp/test-results/out.txt) == "" ]]; then exit 0; else diff --unified tests/centos7/test-out.txt /tmp/test-results/out.txt && exit 1; fi
